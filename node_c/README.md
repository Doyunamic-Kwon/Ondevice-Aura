...
- **임계치**: 0.12 미만이면 스킵 (미세한 표정 노이즈 제거)
- **최소 전송 간격**: 1초 (연속 요청 시에도 최대 1Hz로 제한)
...
### 설계 결정: 텍스트 입력 시에만 LLM 호출

표정 데이터가 변할 때마다 Node B(LLM)를 호출하면 다음 문제가 발생한다.

1. **대화 흐름 파괴**: 사용자가 말하지 않아도 AI가 반응하게 되어 대화가 어색해진다.
2. **LLM 과부하**: 카메라 기반 입력은 초당 수 프레임이 들어오므로, Throttling으로 줄여도 1Hz 수준의 LLM 호출이 지속된다. Jetson 환경에서 Node B는 이를 감당하기 어렵다.
3. **KG 검색 공백**: 텍스트 없이는 ATOMIC KG에서 키워드를 뽑을 수 없어 지식 검색 결과가 항상 0건이다. 의미 있는 맥락 없이 LLM을 호출하는 셈이다.

### 동작 방식

| 수신 데이터 | 동작 |
|------------|------|
| 표정 데이터 (텍스트 없음) | face_va를 NodeC 내부 상태에 누적 저장. Node B 미호출. |
| 표정 데이터 (텍스트 있음) | 전체 파이프라인 가동 → Node B 호출. |
| 음성 데이터 (텍스트 없음) | 스킵. Node B 미호출. |
| 음성 데이터 (텍스트 있음) | 전체 파이프라인 가동 → Node B 호출. |

### 정보 손실 없음

표정 데이터가 Node B를 트리거하지 않아도, 누적된 `last_face_va` 값은 다음 텍스트 턴에 3-모달 융합에 자동으로 참여한다. 사용자가 말하는 순간, 그 이전까지의 표정 상태가 맥락으로 반영된다.

```
[표정만 들어오는 동안]
  face_va 누적 저장 → Node B 미호출

[텍스트가 들어오는 순간]
  KG 검색(텍스트) + Sentiment(텍스트) + 누적된 face_va + voice_va 융합
  → Node B 호출 (풍부한 맥락 포함)
```
...
- **음성 톤 (Voice)**: STT와 함께 Node A가 보내는 Valence-Arousal 값
- **텍스트 감성 (Text)**: Sentiment 모델이 입력 텍스트를 분석한 Valence-Arousal 값

기존 코드는 이 세 신호가 각각 구분 없이 프롬프트에 나열되었고, Alignment Check도 텍스트 vs 표정만 비교했다. 음성 톤은 분석 모듈에서 제외되었다.
...
| 음성 톤 (voice) | 0.4 | 음성의 감정적 컨텐츠를 반영 |
...
기존에는 텍스트 감성 vs 표정 V-A만 비교하는 2-way Alignment였다. 변경 후는 텍스트 감성 vs **융합된 비언어 벡터**를 비교하는 3-way Alignment로 확장되었다. 감정 은폐 감지가 더 견고해진다.
