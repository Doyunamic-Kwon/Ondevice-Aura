...
class VoiceReceiver(aura_pb2_grpc.AuraPerceptionServicer):
    def SendVoicePerception(self, request, context):
        print("!!! VOICE DATA HIT THE HUB !!!")
        print(f"[Voice Received] Text: {request.text} | V:{request.valence} A:{request.arousal}")
        try:
            response = doyun_stub.SendVoicePerception(request, timeout=15)
            ...
            print(f"Voice Forwarding Failed: {e.details()}")
...
def calculate_va(landmarks):
    ...
    v = round(max(-1.0, min(1.0, (w - 0.38) * 5.0)), 2)
...
def main():
    ...
    display_text = "NEUTRAL (V: 0.0, A: 0.0)"
            ...
            if frame_count % 10 == 0:
                ...
                if res.multi_face_landmarks:
                    for landmarks in res.multi_face_landmarks:
                        ...
                        display_text = f"{label.upper()} (V: {v}, A: {a})"
                        ...
                        if label != last_label or v_diff > threshold or a_diff > threshold:
                            ...
                            try:
                                response = doyun_stub.SendFacePerception(candidate, timeout=15)
